...
function stop_collegamento(event)
{
    console.log('hai cliccato il tasto per iniziare a giocare');
    event.preventDefault();
}

const buttons_start = document.querySelectorAll('.flex-benvenuto');
for(const button of buttons_start)
{
    button.addEventListener('click', stop_collegamento);
}